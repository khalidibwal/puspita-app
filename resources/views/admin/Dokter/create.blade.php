<x-app-layout>
    <div class="max-w-md mx-auto bg-white shadow-md rounded-lg overflow-hidden mt-10">
        <div class="p-6">
            <h1 class="text-xl font-semibold mb-4">Add New Dokter</h1>
            <form action="{{ route('dokters.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="alamat" id="alamat" 
                              required 
                              class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring focus:ring-indigo-200 focus:border-indigo-300 p-2"></textarea>
                    @error('alamat')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <button type="submit" class="w-full bg-blue-600 font-semibold py-2 px-4 rounded hover:bg-blue-500 transition">Add Dokter</button>
            </form>
        </div>
    </div>
</x-app-layout>
